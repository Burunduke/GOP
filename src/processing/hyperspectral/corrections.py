...
class HyperspectralCorrections:
    ...
    def radiometric_correction(
        self, image_data: np.ndarray, method: str = "empirical_line"
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            if method not in self.correction_methods:
                self.logger.warning(
                    f"Неизвестный метод коррекции: {method}. Доступные методы: {self.correction_methods}"
                )
                return image_data
    ...
    def calculate_correction_statistics(
        ...
        try:
            ...
            if original_data.shape != corrected_data.shape:
                raise ValueError(
                    "Формы исходных и скорректированных данных должны совпадать"
                )
