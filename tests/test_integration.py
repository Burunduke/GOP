...
"""
    ...
    def test_full_pipeline_integration(
        self, mock_gdal_warp, mock_gdal_open_ortho, mock_gdal_open_hyper
    ):
    ...
    def test_config_integration(self):
        ...
        self.assertEqual(
            config.get("indices.default_indices"), ["GNDVI", "NDWI", "MCARI"]
        )
    ...
    def test_indices_integration(self):
        ...
        result = calculator.calculate(
            spectral_data, ["GNDVI", "NDWI"], segmentation_mask
        )
    ...
    def test_error_propagation_integration(self):
        ...
        with self.assertRaises(FileNotFoundError):
            pipeline.process(
                input_path="/nonexistent/file.bil", output_dir=self.test_output_dir
            )
    ...
    def test_scientific_analysis_integration(self):
        ...
        with patch.object(pipeline, "_perform_scientific_analysis") as mock_scientific:
            ...
            result = pipeline._perform_scientific_analysis(
                indices_results, np.random.randint(0, 5, (100, 100))
            )
...
""")
        ...
        with patch.object(pipeline, "process") as mock_process:
            mock_process.return_value = {
                ...
                "plant_condition": {
                    "classification": {"class": "healthy", "score": 0.8}
                },
            }
            ...
            for test_file in test_files:
                file_output_dir = os.path.join(
                    self.test_output_dir, f"output_{os.path.basename(test_file)}"
                )
                result = pipeline.process(
                    input_path=test_file, output_dir=file_output_dir
                )
