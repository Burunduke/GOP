...
class TestVisualization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.plant_condition_data = {
            ...
            "statistics": {"overall": {"mean": 0.70, "std": 0.05}},
        }
    ...
    def test_visualize_indices(self, mock_subplots, mock_show, mock_savefig):
        ...
        mock_fig = MagicMock()
        mock_axes = [
            MagicMock() for _ in range(len(self.indices_dict) + 1)
        ]  # +1 для colorbar
    ...
    def test_create_comparison_plot(self, mock_subplots, mock_show, mock_savefig):
        ...
        result = create_comparison_plot(
            self.original_image, self.segmentation_mask, self.indices_dict, output_path
        )
    ...
    def test_create_comparison_plot_invalid_image(self):
        ...
        with self.assertRaises(ValueError):
            create_comparison_plot(
                invalid_image, self.segmentation_mask, self.indices_dict, self.temp_dir
            )
    ...
    def test_create_comparison_plot_invalid_mask(self):
        ...
        with self.assertRaises(ValueError):
            create_comparison_plot(
                self.original_image, invalid_mask, self.indices_dict, self.temp_dir
            )
