...
"""
    ...
    def test_process_invalid_input_path(self):
        ...
        with self.assertRaises(FileNotFoundError):
            self.pipeline.process(
                input_path="nonexistent_file.bil", output_dir=self.test_output_dir
            )
    ...
    def test_process_with_custom_indices(self):
        ...
        with patch.object(
            self.pipeline.index_calculator, "calculate"
        ) as mock_calculate:
    ...
    def test_save_results(self):
        ...
        test_results = {
            ...
            "plant_condition": {"classification": {"class": "healthy", "score": 0.85}},
        }
    ...
    def test_load_results(self):
        ...
        self.assertEqual(
            loaded_results["orthophoto_path"], test_results["orthophoto_path"]
        )
        self.assertEqual(loaded_results["indices"], test_results["indices"])
        self.assertEqual(
            loaded_results["plant_condition"], test_results["plant_condition"]
        )
    ...
    def test_cancel_processing(self):
        ...
        def process_in_background():
            try:
                self.pipeline.process(
                    input_path=self.test_bil_path, output_dir=self.test_output_dir
                )
            except Exception:
    ...
    def test_export_scientific_data_after_processing(self, mock_process):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.test_output_dir, "scientific_data.json"))
        )
        self.assertTrue(
            os.path.exists(
                os.path.join(self.test_output_dir, "processing_metadata.json")
            )
        )
    ...
    def test_find_strong_correlations_empty_data(self):
        ...
        correlations = self.pipeline._find_strong_correlations(
            empty_indices, threshold=0.5
        )
    ...
    def test_save_scientific_report_invalid_path(self):
        ...
        analysis = {"indices": {"GNDVI": {"mean": 0.5, "std": 0.1}}, "correlations": []}
    ...
    def test_process_with_memory_efficiency(self):
        ...
        with patch.object(
            self.pipeline, "_preprocess_hyperspectral"
        ) as mock_preprocess:
            ...
            with patch.object(self.pipeline, "_create_orthophoto") as mock_orthophoto:
                mock_orthophoto.return_value = os.path.join(
                    self.test_output_dir, "orthophoto.tif"
                )
                ...
                with patch.object(self.pipeline, "_segment_image") as mock_segment:
                    ...
                    with patch.object(
                        self.pipeline, "_calculate_indices"
                    ) as mock_calculate:
                        mock_calculate.return_value = {
                            "GNDVI": np.random.rand(100, 100)
                        }
                        ...
                        with patch.object(
                            self.pipeline, "_assess_plant_condition"
                        ) as mock_assess:
                            mock_assess.return_value = {
                                "classification": {"class": "healthy", "score": 0.85}
                            }
